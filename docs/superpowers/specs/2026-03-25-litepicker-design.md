...
| `test_medium_request` | `[{"role":"user","content":"debug and refactor this async function to use dependency injection, returning a structured JSON response"}]` | tier == `"medium"` AND `0.30 <= confidence < 0.50`. Assert both tier and confidence range to make the test self-validating against threshold changes. (Note: the shorter string "refactor this function..." scores ~0.25 confidence — below `SIMPLE_BOUNDARY` — so the longer string is required to land in the medium range.) |
...
| `test_tool_use_not_simple` | `messages=[{"role":"user","content":"debug and refactor this async function to use dependency injection, returning a structured JSON response"}]`, `tools=[{"type":"function","function":{"name":"foo","parameters":{}}}]` | tier != `"simple"`. Uses the same content as `test_medium_request` (known to score above `SIMPLE_BOUNDARY`) to guarantee the message itself is non-simple; tools arg exercises the `tool_use_required` code path. |
